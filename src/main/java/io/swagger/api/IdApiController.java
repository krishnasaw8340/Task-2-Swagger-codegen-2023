...
public class IdApiController implements IdApi {
    ...
    public ResponseEntity<Mdata> idGet(@ApiParam(value = "ID of the data",required=true) @PathVariable("id") Integer id) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Mdata>(objectMapper.readValue("{\"empty\": false}", Mdata.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<Mdata>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<Mdata>(HttpStatus.NOT_IMPLEMENTED);
    }
